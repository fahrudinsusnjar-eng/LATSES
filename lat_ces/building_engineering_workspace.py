...
This module is deliberately UI-first: it gives the Building Model one coherent
engineering workspace without duplicating the underlying scientific engines.
The existing BuildingModel remains the source of truth; this view only exposes
navigation, drafting context, airflow visualization, and analysis summaries.
"""
...
class BuildingEngineeringWorkspace(tk.Frame):
    """A single-window engineering workspace around the existing BuildingModel."""

    def __init__(self, master: tk.Misc, **kwargs) -> None:
        super().__init__(master, **kwargs)
        self.configure(padx=0, pady=0)
        self.active = tk.StringVar(value="model")
        self.view = tk.StringVar(value="TLOCRT")
        self._build()
        self._show("model")
    ...
    def _nav_button(self, parent: ttk.Frame, module: EngineeringModule) -> None:
        button = ttk.Button(parent, text=f"{module.icon}  {module.title}", width=22, command=lambda k=module.key: self._show(k))
    ...
    def _cards(self, values: list[tuple[str, str, str]]) -> None:
        ...
        for i, (title, value, note) in enumerate(values):
            ...
            ttk.Label(box, text=note, foreground="#64748b", wraplength=170).pack(anchor="w", pady=(4, 0))
    ...
    def _model(self) -> None:
        ...
        for label, value in data:
            ttk.Label(box, text=label, font=("Segoe UI", 9, "bold")).grid(sticky="w", pady=4)
            ttk.Label(box, text=value).grid(sticky="w", padx=(120, 0), row=box.grid_size()[1] - 1)
        ...
        ttk.Label(info, text="Otvor vrata/prozora je geometrijski otvor. Zid se automatski nastavlja iznad otvora do visine etaže.", wraplength=330).pack(anchor="w", pady=(14, 0))
    ...
    def _airflow(self) -> None:
        ...
        # distributed, multi-source flow instead of one strong jet
        ...
        for label, value in (("Svježi zrak", "30–35 m³/h"), ("Ciljna brzina", "≤ 0.05 m/s"), ("Zona", "0.70–1.70 m"), ("Smjer", "dovod → prostor → odsis"), ("CO₂", "model / senzor"), ("VOC / HCHO", "model / senzor"), ("Uzgon", "AKTIVAN MODEL")):
    ...
    def _system(self, key: str) -> None:
        ...
        self._cards([("GEOMETRIJA", "LINKED", "zidovi, prostorije, otvori"), ("PRORAČUN", "ENGINE", "scientific engine priključen"), ("PRIJEDLOG", "AI", "predlaže, ne odlučuje")])
    ...
    def _analysis(self, key: str) -> None:
        ...
        ttk.Label(body, text="Rezultat se izvodi iz BuildingModela. Ako nedostaje ulazni podatak, LATCES mora to jasno prijaviti umjesto da izmišlja vrijednost.", wraplength=650).pack(anchor="w", pady=(8, 18))
