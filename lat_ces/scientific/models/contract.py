...
class ScientificModelContract:
            ...
            if not item.description.strip():
                issues.append(
                    ContractIssue(
                        ...
                        f"Input '{item.name}' must have a description.",
                    )
                )
            ...
            if not item.description.strip():
                issues.append(
                    ContractIssue(
                        ...
                        f"Output '{item.name}' must have a description.",
                    )
                )
        ...
        for item in (*self.metadata.inputs, *self.metadata.outputs):
            if not hasattr(item.dimension, "is_compatible"):
                issues.append(
                    ContractIssue(
                        "INVALID_DIMENSION",
                        f"'{item.name}' does not contain a valid Dimension.",
                    )
                )
    ...
    def validate(self) -> tuple[ContractIssue, ...]:
        """Return all structural contract violations."""
    ...
    def is_valid(self) -> bool:
        """Return True when no contract violations exist."""

        return not self.validate()

    @staticmethod
    def _check_unique_names(
        items: Iterable[ModelInput | ModelOutput],
        *,
        code: str,
        kind: str,
    ) -> tuple[ContractIssue, ...]:
        ...
        for item in items:
            if item.name in seen:
                issues.append(
                    ContractIssue(
                        code,
                        f"Duplicate {kind} name: '{item.name}'.",
                    )
                )
            seen.add(item.name)
