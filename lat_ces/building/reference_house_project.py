...
def _add_reference_rooms(level: Level, room_data: list[dict]) -> None:
    ...
    for data in room_data:
        ...
        level.add_room(Room(
            ...
            footprint=Box3D(origin=Point3D(0.0, y, level.elevation), length=length, width=width, height=height),
        ))
        y += width
...
def _populate_reference_engineering_inputs(workflow: BuildingWorkflow, data: dict) -> None:
    ...
    for index, source_room_id in enumerate(ventilation_rooms):
        ...
        registry.add_ventilation_opening(VentilationOpening(
            id=f"VO-{index + 1:02d}", room_id=room_map[source_room_id].room_id, kind=kind,
            diameter_m=0.25, design_velocity_m_s=0.50, elevation_m=0.70 if kind == "supply" else 2.40,
            x_m=1.0, y_m=1.0 + index,
        ))

    for index, source_room_id in enumerate(["P-KIT", "P-BTH", "S1-BTH", "S2-BTH"]):
        room_id = room_map[source_room_id].room_id
        registry.add_water_branch(WaterBranch(
            id=f"CW-{index + 1:02d}", room_id=room_id, service="cold_water", diameter_m=0.020,
            design_flow_m3_s=0.00020, length_m=5.0 + index, x1_m=0.0, y1_m=float(index), x2_m=5.0, y2_m=float(index),
        ))
        registry.add_water_branch(WaterBranch(
            id=f"DR-{index + 1:02d}", room_id=room_id, service="drain", diameter_m=0.050,
            design_flow_m3_s=0.00050, length_m=4.0 + index, x1_m=5.0, y1_m=float(index), x2_m=9.0, y2_m=float(index),
        ))
    ...
    for circuit in data["heating"]["circuits"]:
        for source_room_id in circuit["rooms"]:
            ...
            registry.add_heating_zone(HeatingZone(
                id=f"{circuit['id']}-{source_room_id}", room_id=room_map[source_room_id].room_id,
                emitter_type=str(circuit["type"]), design_supply_temp_c=float(circuit["supply_c"]),
                design_return_temp_c=float(circuit["return_c"]), target_indoor_temp_c=20.0, room_heat_load_w=load_w,
            ))
...
def build_reference_house_workflow() -> BuildingWorkflow:
    ...
    wall_construction = WallConstructionSpec(
        block_brand=str(wall.get("masonry_block", "")), wall_thickness_m=0.25,
        insulation_type=str(wall.get("insulation", "")), insulation_thickness_m=float(wall.get("insulation_thickness_m", 0.0)),
        exterior_cladding=str(wall.get("facade_finish", "")), interior_cladding=str(wall.get("interior_finish", "")),
    ...
    for level_data in data["levels"]:
        ...
        project.levels.append(LevelProjectSpec(
            name=str(level_data["name"]), height_m=float(dimensions["level_height_m"]), length_m=float(dimensions["length_m"]), width_m=float(dimensions["width_m"]),
            construction=wall_construction, cladding=str(wall.get("facade_finish", "")),
            joinery=JoinerySpec(material=str(joinery.get("default_frame", "")), glazing="3 stakla / argon / Low-E / warm edge", frame_type=str(joinery.get("default_frame", "")), thermal_transmittance_w_m2k=0.7, opening_count=0),
            rooms=rooms, finalized=False,
        ))
    workflow = BuildingWorkflow(model=model, project_spec=project, current_step=3)
    previous = None
    for level_data in data["levels"]:
        joinery = data.get("joinery", {})
        level = model.add_level(Level(
            name=str(level_data["name"]), elevation=0.0 if previous is None else previous.top_elevation, height=float(dimensions["level_height_m"]),
            length_m=float(dimensions["length_m"]), width_m=float(dimensions["width_m"]), wall_construction=str(wall.get("masonry_block", "")),
            insulation=str(wall.get("insulation", "")), cladding=str(wall.get("facade_finish", "")), joinery=str(joinery.get("default_frame", "")),
            facade_finish=str(wall.get("facade_finish", "")), insulation_material=str(wall.get("insulation", "")),
            insulation_thickness_m=float(wall.get("insulation_thickness_m", 0.0)), interior_plaster_material=str(wall.get("interior_finish", "")),
            interior_plaster_thickness_m=float(wall.get("interior_finish_thickness_m", 0.0)), dead_load_kpa=float(level_data.get("loads", {}).get("dead_kpa", 0.0)),
    ...
    roof_data = data["roof"]
    model.set_roof(Roof(
        roof_type=str(roof_data["type"]), construction="Drvena krovna građa", covering=str(roof_data.get("covering", "")),
        substructure=f"Rog {roof_data.get('rafter_section_mm', [100, 200])[0]}×{roof_data.get('rafter_section_mm', [100, 200])[1]} mm",
        support=str(roof_data.get("ridge_direction", "")), length_m=float(dimensions["length_m"]), width_m=float(dimensions["width_m"]),
        slope_deg=float(roof_data.get("slope_deg", 0.0)), height_m=0.0,
    ))
