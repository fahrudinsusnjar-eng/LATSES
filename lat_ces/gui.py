...
class LATCESApp(tk.Tk):
    ...
    def _build_side_panel(self, side: ttk.Frame) -> None:
        ...
        self.step_box = ttk.LabelFrame(side, text="Trenutni korak", padding=10)
        self.step_box.pack(fill="x", pady=(14, 0))
        self.step_title = ttk.Label(self.step_box, text="1. Tlocrt", font=("Segoe UI", 14, "bold"))
        self.step_title.pack(anchor="w")
        self.step_info = ttk.Label(self.step_box, text="Nacrtaj ili izmijeni zidove.", wraplength=290)
        self.step_info.pack(anchor="w", pady=(4, 8))
        self.step_controls = ttk.Frame(self.step_box)
        self.step_controls.pack(fill="x")

        summary = ttk.LabelFrame(side, text="Building Model", padding=10)
        summary.pack(fill="x", pady=(14, 0))
        self.summary_text = tk.Text(summary, height=7, width=36, wrap="word", state="disabled")
    ...
    def goto_step(self) -> None:
        step = self.step_var.get()
        if step == 2 and not self.workflow.model.levels:
            self.workflow.set_floor_plan(self.floor_plan)
        if step == 3:
    ...
    def configure_step(self, step: int) -> None:
        for child in self.step_controls.winfo_children():
            child.destroy()
        if step == 1:
            self.workspace.configure(text="Tlocrt")
            self.step_title.configure(text="1. Tlocrt")
            self.step_info.configure(text="Nacrtaj i izmijeni zidove. Vrata i prozori dolaze u koraku 3.")
            self._build_workspace_toolbar()
        elif step == 2:
            self.workspace.configure(text="Visina / spratnost")
            self.step_title.configure(text="2. Visina / spratnost")
            self.step_info.configure(text="Definiši visinu etaže i dodaj spratove. Svi nivoi ostaju u istom BuildingModelu.")
            ...
            ttk.Button(self.step_controls, text="Dodaj sprat", command=self.add_level).pack(fill="x", pady=3)
            ttk.Button(self.step_controls, text="Sljedeće: Otvori →", command=lambda: self.step_var.set(3) or self.goto_step()).pack(fill="x", pady=(8, 0))
        elif step == 3:
            self.workspace.configure(text="Otvori — vrata i prozori")
            self.step_title.configure(text="3. Otvori")
            self.step_info.configure(text="Na odabranom zidu dodaj vrata ili prozor. Otvori su dio fizičkog modela.")
            ttk.Button(self.step_controls, text="Vrata", command=lambda: self.editor.set_tool("door")).pack(fill="x", pady=3)
            ttk.Button(self.step_controls, text="Prozor", command=lambda: self.editor.set_tool("window")).pack(fill="x", pady=3)
            ttk.Button(self.step_controls, text="Sljedeće: 3D model →", command=lambda: self.step_var.set(4) or self.goto_step()).pack(fill="x", pady=(8, 0))
        else:
            self.workspace.configure(text="3D Building Model")
            self.step_title.configure(text="4. 3D model")
            self.step_info.configure(text="3D geometrija je izvedena iz iste 2D FloorPlan + etaža. Nema paralelnog modela.")
            ttk.Button(self.step_controls, text="← Nazad na tlocrt", command=lambda: self.step_var.set(1) or self.goto_step()).pack(fill="x")

        for child in getattr(self, "step_controls", ()).winfo_children() if hasattr(self, "step_controls") else ():
            _ = child
    ...
    def apply_height(self) -> None:
        try:
            self.workflow.set_active_level_height(float(self.height_var.get().strip()))
    ...
    def add_level(self) -> None:
        ...
        if height is None:
            return
        level = self.workflow.add_level(name, height)
        level.set_floor_plan(build_default_floor_plan())
        self.floor_plan = level.floor_plan
        self.height_var.set(f"{height:.2f}")
        self.refresh_plan()
        self.status_var.set(f"Dodan {level.name}, elevacija {level.elevation:.2f} m")

    def new_project(self) -> None:
        self.workflow = new_workflow()
        self.floor_plan = self.workflow.floor_plan
        self.height_var.set("2.80")
        self.step_var.set(1)
        self.configure_step(1)
        self.refresh_plan()
        self.status_var.set("Novi projekat — Korak 1: Tlocrt")
    ...
    def nearest_wall(self, point: Point2D, tolerance_m: float = 0.30) -> Wall | None:
        ...
        for wall in self.floor_plan.walls.values():
            ...
            ls = dx * dx + dy * dy
            if ls == 0:
                distance = math.hypot(point.x - wall.segment.start.x, point.y - wall.segment.start.y)
            else:
                t = max(0.0, min(1.0, ((point.x - wall.segment.start.x) * dx + (point.y - wall.segment.start.y) * dy) / ls))
                px = wall.segment.start.x + t * dx
                py = wall.segment.start.y + t * dy
    ...
    def model_to_canvas(self, point: Point2D) -> tuple[float, float]:
        ...
        ox, oy = (width - 12.0 * scale) / 2, (height - 8.0 * scale) / 2
    ...
    def draw_floor_plan(self) -> None:
        ...
        for x in range(13):
            px = ox + x * scale
            self.canvas.create_line(px, oy, px, oy + 8 * scale, fill="#edf0f2")
        for y in range(9):
            py = oy + y * scale
            self.canvas.create_line(ox, py, ox + 12 * scale, py, fill="#edf0f2")
        for wall in self.floor_plan.walls.values():
            x1, y1 = self.model_to_canvas(wall.segment.start)
            x2, y2 = self.model_to_canvas(wall.segment.end)
            self.canvas.create_line(x1, y1, x2, y2, width=max(4, int(wall.thickness * scale)), fill="#374151")
            for opening in wall.openings:
                ratio = opening.offset / wall.segment.length
                cx = wall.segment.start.x + (wall.segment.end.x - wall.segment.start.x) * ratio
                cy = wall.segment.start.y + (wall.segment.end.y - wall.segment.start.y) * ratio
                ex = wall.segment.end.x - wall.segment.start.x
                ey = wall.segment.end.y - wall.segment.start.y
                l = wall.segment.length
                sx = cx + ex / l * opening.width
                sy = cy + ey / l * opening.width
                a, b = self.model_to_canvas(Point2D(cx, cy)), self.model_to_canvas(Point2D(sx, sy))
                self.canvas.create_line(a[0], a[1], b[0], b[1], width=10, fill="#ffffff")
                self.canvas.create_line(a[0], a[1], b[0], b[1], width=3, fill="#2563eb" if opening.kind == "window" else "#b45309")
        self.canvas.create_text(width - 18, 18, text=f"Nivo: {self.active_level.name}  |  h={self.active_level.height:.2f} m", anchor="ne", fill="#5f6368")
        self.canvas.create_text(width - 18, 38, text="Sjever ↑", anchor="ne", fill="#5f6368")

    def draw_3d(self) -> None:
        self.canvas.delete("all")
        try:
            geometries: tuple[LevelGeometry3D, ...] = self.workflow.advance_to_3d()
        except ValueError as exc:
            self.canvas.create_text(20, 20, text=str(exc), anchor="nw")
            return
        width, height = max(self.canvas.winfo_width(), 400), max(self.canvas.winfo_height(), 300)
        scale = min((width - 160) / 12.0, (height - 180) / 8.0)
        ox, oy = 90, height - 90
        z_scale = 35
        for level_index, geometry in enumerate(geometries):
            z = sum(g.height for g in geometries[:level_index])
            for wall in geometry.walls:
                x1 = ox + wall.x1 * scale
                y1 = oy - wall.y1 * scale - z * z_scale
                x2 = ox + wall.x2 * scale
                y2 = oy - wall.y2 * scale - z * z_scale
                top1, top2 = y1 - wall.height * z_scale, y2 - wall.height * z_scale
                self.canvas.create_polygon((x1, y1, x2, y2, top2, top1), fill="#d1d5db", outline="#374151")
        self.canvas.create_text(20, 20, text="3D Building Model — izveden iz FloorPlan + Level", anchor="nw", font=("Segoe UI", 12, "bold"))
        total_height = sum(g.height for g in geometries)
        self.canvas.create_text(20, 44, text=f"Etaže: {len(geometries)}   Ukupna visina: {total_height:.2f} m", anchor="nw", fill="#5f6368")

    def update_summary(self) -> None:
        summary = self.workflow.summary()
        openings = sum(len(w.openings) for l in self.workflow.model.levels.values() if l.floor_plan for w in l.floor_plan.walls.values())
        text = (
            f"Objekat: {summary['model']}\n"
            f"Etaže: {summary['levels']}\n"
            f"Aktivna etaža: {summary['active_level']}\n"
            f"Zidovi: {self.floor_plan.wall_count}\n"
            f"Otvori: {openings}\n"
            f"Površina: {summary['floor_area_m2']:.2f} m²\n"
            f"Volumen: {summary['volume_m3']:.2f} m³"
        )
    ...
    def validate_model(self) -> None:
        ...
        if findings:
            messagebox.showwarning("LAT-CES", "\n".join(findings), parent=self)
            self.status_var.set(f"Provjera: {len(findings)} nalaza")
        else:
            messagebox.showinfo("LAT-CES", "Building Model je validan.", parent=self)
            self.status_var.set("Building Model: validan")
    ...
    def export_project(self) -> None:
        ...
        if not target:
            return
        payload = {
            "name": self.workflow.model.name,
            "model_id": self.workflow.model.model_id,
            "levels": [
                {
                    "name": level.name,
                    ...
                    "walls": [
                        {
                            "name": wall.name,
                            "start": {"x_m": wall.segment.start.x, "y_m": wall.segment.start.y},
                            "end": {"x_m": wall.segment.end.x, "y_m": wall.segment.end.y},
                            "thickness_m": wall.thickness,
                            "openings": [{"kind": o.kind, "offset_m": o.offset, "width_m": o.width} for o in wall.openings],
                        }
        ...
        self.status_var.set(f"Building Model sačuvan: {target}")
    ...
    def open_analysis(self) -> None:
        ...
        ttk.Button(cfg, text="Browse…", command=lambda: path_var.set(filedialog.askopenfilename(filetypes=(("JSON files", "*.json"),)))).grid(row=0, column=2)
