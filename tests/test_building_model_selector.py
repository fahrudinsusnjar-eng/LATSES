...
def test_selector_switches_between_registered_canonical_models() -> None:
    ...
    assert option.description == "Testni model"
    assert option.source == "test"

    assert selector.select(second.model_id) is second
    assert selector.selected is second


def test_selector_does_not_allow_removing_active_model() -> None:
    model = BuildingModel(name="Projekt")
    selector = BuildingModelSelector(model)
