...
def test_quantity_takeoff_uses_canonical_geometry():
    ...
    level = Level("Prizemlje", elevation=0.0, height=2.8, floor_plan=plan, length_m=4.0, width_m=3.0)
    ...
    assert qto.floor_area_m2 == 12.0
    assert qto.volume_m3 == 33.6
    assert qto.wall_length_m == 4.0
    assert qto.roof_plan_area_m2 == 12.0
