...
def test_build_geometry_extrudes_existing_floor_plan():
    ...
    assert geometry[0].walls[0].length == pytest.approx(4.0)
    assert geometry[0].walls[0].gross_area == pytest.approx(12.0)
    assert geometry[0].walls[0].volume == pytest.approx(2.4)


def test_material_layer_requires_positive_thickness():
    layer = MaterialLayer("Concrete", 0.20)
    assert layer.material == "Concrete"
    assert layer.thickness == pytest.approx(0.20)
